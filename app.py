...
def guardar_base_con_reintentos(nuevo_registro, modo, usuario_actual, max_reintentos=3):
    ...
    for intento in range(max_reintentos):
        try:
            ...
            df_actual.columns = [str(col).strip() for col in df_actual.columns]

            sot_id = str(nuevo_registro['SOT']).strip()

            if modo == "Crear Nuevo Registro":
                if 'SOT' in df_actual.columns and sot_id in df_actual['SOT'].astype(str).str.strip().values:
                    return False, f"La SOT {sot_id} ya fue registrada previamente."
                df_actual = pd.concat([df_actual, pd.DataFrame([nuevo_registro])], ignore_index=True)
            else:
                filtro_sot = df_actual['SOT'].astype(str).str.strip() == sot_id
                for key, val in nuevo_registro.items():
                    if key in df_actual.columns:
                        df_actual.loc[filtro_sot, key] = str(val)

            buffer = io.BytesIO()
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                df_actual.to_excel(writer, sheet_name=nombre_hoja, index=False)
...
def cargar_opciones_config():
    ...
    if content:
        xls = pd.ExcelFile(io.BytesIO(content))
        def obtener_lista(hoja, columna):
            if hoja in xls.sheet_names:
                df_h = pd.read_excel(xls, hoja)
                df_h.columns = [str(c).strip() for c in df_h.columns]
                if columna in df_h.columns:
                    return df_h[columna].dropna().astype(str).str.strip().tolist()
            return []

        config['TABULACION'] = obtener_lista('TABULACION', 'TABULACION')
        config['TIPO_DE_DEV'] = obtener_lista('TIPO _DE_ DEV', 'TIPO _DE_ DEV')
        config['MOTIVO_DE_DEV'] = obtener_lista('MOTIVO_DE_DEV', 'MOTIVO_DE_DEV')
        config['VERIFICACION_BO'] = obtener_lista('VERIFICACION_BO', 'VERIFICACION_BO')
        config['LLAMADA_BO'] = obtener_lista('LLAMADO_BO', 'LLAMADA BO')
...
if menu_sel == "Gestión de Grabaciones":
    ...
    with st.form("form_grabacion"):
        ...
        with col1:
            ...
            fec_gen_sot = st.text_input("FEC_GEN_SOT", value=str(registro_actual.get('FEC_GEN_SOT', '')))
            fecha_instalacion = st.text_input("FECHA_INSTALACION", value=str(registro_actual.get('FECHA_INSTALACION', '')))
            fecha_rechazo = st.text_input("FECHA_RECHAZO", value=str(registro_actual.get('FECHA_RECHAZO', '')))
            fecha_agenda = st.text_input("FECHA_AGENDA", value=str(registro_actual.get('FECHA_AGENDA', '')))
        ...
        with col3:
            ...
            sot_correcta = st.text_input("SOT_CORRECTA", value=str(registro_actual.get('SOT_CORRECTA', '')))
    ...
    if btn_guardar:
        if not sot:
            ...
            nuevo_registro = {
                'SOT': sot, 'FEC_GEN_SOT': fec_gen_sot, 'FECHA_INSTALACION': fecha_instalacion,
                'FECHA_RECHAZO': fecha_rechazo, 'FECHA_AGENDA': fecha_agenda, 'PLAN': plan,
                'CARGO_FIJO_CON_IGV': cargo_fijo, 'DEPARTAMENTO_INSTALACION': dep_inst,
                'DISTRITO_INSTALACION': dist_inst, 'PROVINCIA_INSTALACION': prov_inst,
                'CONTRATA': contrata, 'PUNTO_VENTA': punto_venta, 'DISTRIBUIDOR': distribuidor,
                'ESTADO_CONTACTO': estado_contacto, 'TECNICO': tecnico,
                'FEC_PROGRAMACION_TOA': fec_prog_toa, 'FRANJA': franja, 'ESTADO_TOA': estado_toa,
                'NUMERO_DE_SEC': numero_sec, 'ARBITRAJE': arbitraje, 'SOT_INICIAL': sot_inicial,
                ...
                'MOTIVO_PENALIDAD': motivo_penalidad, 'ALTAS': altas, 'ZONAL_VDD': zonal_vdd,
                'VENDEDOR_REAL': vendedor_real, 'SUPERVISOR': supervisor, 'MODALIDAD_VDD': modalidad_vdd
            }
            éxito, msj = guardar_base_con_reintentos(nuevo_registro, modo, st.session_state.username)
            if éxito:
                st.success(f"¡SOT {sot} guardada con éxito por {st.session_state.nombre}!")
                time.sleep(1)
                st.rerun()
            else:
